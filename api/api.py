...
def startup():
    with get_connection() as connection:
        with connection.cursor() as cursor:
            ...
            # Application identity is generated by ControlPlane and
            # persisted by workflow name. A workflow name maps to exactly
            ...
            cursor.execute("""
                DROP INDEX IF EXISTS idx_applications_session_name
            """)
...
def create_application(payload: ApplicationCreate):
    """
    Resolve a workflow name to its persistent application UUID.

    The application UUID is generated by ControlPlane on first creation.
    Repeated requests for the same workflow name return the existing UUID.

    The database UNIQUE(name) constraint is the final source of truth, so
    concurrent SDK processes cannot create duplicate applications.
    """
    ...
    with get_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                VALUES (%s, %s)
                ON CONFLICT (name)
                DO UPDATE SET name = applications.name
                RETURNING
                    id,
                    name,
                    created_at,
